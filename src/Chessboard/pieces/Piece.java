...
public abstract class Piece implements Cloneable {
	...
	protected ArrayList createStraightMovements(Chessboard chessboard) {
		...
		for (int i = 1; i <= Math.max(startRow, startCol); i++) {
			...
			if (!rightBlocked && startCol + i < 8) {
				if (chessboard.getSquareContents(startRow, startCol + i) == ' ') {
					movements.add(new Movement(0, position, startRow * 8 + (startCol + i)));
			...
			if (!downBlocked && startRow + i < 8) {
				if (chessboard.getSquareContents(startRow + i, startCol) == ' ') {
					movements.add(new Movement(0, position, (startRow + i) * 8 + startCol));
	...
	protected ArrayList createDiagonalMovements(Chessboard chessboard) {
		ArrayList<Movement> movements = new ArrayList<>();
		int row = position / 8;
		int col = position % 8;
		int maxMovement = 0;
		...
		for (int i = 1; i < maxMovement; i++) {
			...
			if (!neBlocked && row - i >= 0 && col + i < 8) {
				if (chessboard.getSquareContents(row-i, col+i) == ' ') {
					movements.add(new Movement(0, position, nePosition));
			...
			if (!seBlocked && row + i < 8 && col + i < 8) {
				if (chessboard.getSquareContents(row+i, col+i) == ' ') {
					movements.add(new Movement(0, position, sePosition));
			...
			if (!swBlocked && row + i < 8 && col - i >= 0) {
				if (chessboard.getSquareContents(row+i, col-i) == ' ') {
					movements.add(new Movement(0, position, swPosition));
			...
			if (!nwBlocked && row - i >= 0 && col - i >= 0) {
				if (chessboard.getSquareContents(row-i, col-i) == ' ') {
					movements.add(new Movement(0, position, nwPosition));
